...
msw <- read.csv('../out/BC_Municipal_Solid_Waste_Disposal_viz.csv'
...
max_year_rate <- msw$Disposal_Rate_kg[msw$Regional_District == "British Columbia" & 
                                        msw$Year == max_year]

max_year_pop <- msw$Population[msw$Regional_District == "British Columbia" & 
                                        msw$Year == max_year]

max_year_total <- msw$Total_Disposed_Tonnes[msw$Regional_District == "British Columbia" & 
                                        msw$Year == max_year]

comp_year <- 2012

comp_year_rate <- msw$Disposal_Rate_kg[msw$Regional_District == "British Columbia" & 
                                        msw$Year == comp_year]
...
msw.bc <- msw[msw$Regional_District == "British Columbia",]

p <- ggplot(data = msw.bc, aes(x = Year, y = Disposal_Rate_kg))
...
scale.y <- scale_y_continuous(breaks = seq(0, max(msw.bc$Disposal_Rate_kg
                                               , na.rm = TRUE) + 100, by = 100)
                             , limits = c(0, max(msw.bc$Disposal_Rate_kg
                                                , na.rm = TRUE) + 50)
                             , expand = c(0,0))

scale.x <- scale_x_continuous(breaks = seq(min(msw.bc$Year), max(msw.bc$Year), by = 5))
...
msw.rd <- msw[msw$Regional_District != "British Columbia",]

ordered_by_pop <- msw.rd$Regional_District[msw.rd$Year == max_year][order(msw.rd$Population[msw.rd$Year == max_year])]

msw.rd$Regional_District <- factor(msw.rd$Regional_District, levels = as.character(ordered_by_pop))

msw.rd$Population_Category <- cut(msw.rd$Population, 
                                  breaks = c(0, 10000, 50000, 100000, 500000, 1e6, Inf), 
                                  labels = c("< 10,000", "< 50,000", "< 100,000", "< 500,000", 
                                             "< 1,000,000", "> 1,000,000"), 
                                  include.lowest = TRUE)

ggplot(msw.rd, aes(x = Year, y = Disposal_Rate_kg, fill = Population_Category)) +
...
Published and Available On-Line at Environmental Reporting BC (March 2018):
