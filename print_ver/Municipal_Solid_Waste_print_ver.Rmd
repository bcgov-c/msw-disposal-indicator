...
min_rate <- min(BC_data$Disposal_Rate_kg)

min_rate_year <- max(BC_data$Year[BC_data$Disposal_Rate_kg == min_rate])
```
...
-   In `r max_year` British Columbians disposed of an
    ...
    comp_year_rate))` kg per person from `r comp_year`, though higher 
    than the low of `r min_rate` kg per persion in `r min_rate_year`.
-   Provincial initiatives which support decreases in waste disposal
    rates include the CleanBC [Plastics Action Plan](https://cleanbc.gov.bc.ca/app/uploads/sites/436/2019/08/CleanBC_PlasticsActionPlan_ConsultationPaper_07252019_B.pdf), [organics
    ...
    responsibility (recycling)](https://www2.gov.bc.ca/gov/content/environment/waste-management/recycling/product-stewardship) programs, and funding for organics infrastructure and collection.
...
Published and Available On-Line at Environmental Reporting BC (September 2021):
