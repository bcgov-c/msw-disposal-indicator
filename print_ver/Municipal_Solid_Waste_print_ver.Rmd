...
title: "Municipal Solid Waste Disposal in B.C. (1990-2017)"
...
-   Approximately `r round(max_year_total / 1e6, 1)` million tonnes of municipal 
    solid waste was disposed of in B.C. in `r max_year`. While there has been an 
    overall decreasing trend in municipal solid waste disposal rates since the
...
-   In `r max_year` British Columbians disposed of an average of 
    `r round(max_year_rate)` kg of municipal solid waste per person---down 
    `r abs(round(max_year_rate - comp_year_rate))` kg per person from `r comp_year`.
-   While BC is still experiencing an overall decreasing trend in
    disposal rates, `r max_year` is the first year (since the current tracking
    method began in `r comp_year`) where an increase over the previous year was
    observed. Potential reasons for this include: a strong economy which
    has led to higher consumption and tourism, and a strong real estate
    market resulting in more waste generated from moving and the
    renovation and/or demolition of old homes.
-   Provincial initiatives which are expected to decrease waste disposal
    rates despite strong economic cycles include [organics
    diversion](https://www2.gov.bc.ca/gov/content/environment/waste-management/food-and-organic-waste/organic-waste-diversion),
    [food waste
    ...
    responsibility](https://www2.gov.bc.ca/gov/content/environment/waste-management/recycling/product-stewardship)
    programs.
-   British Columbia has set provincial waste disposal targets with a
    ...
    to 350 kg per person by 2020/21.^[1](#fn1)^
-   Variability in disposal rates from one region to the next is
    ...
    area and the capacity and infrastructure in place in a region. While
    disposal rate data don't tell the whole story, they are useful in
...
-   Regional District population estimates are sourced from [BC
    ...
    Squamish-Lillooet and East Kootenay population estimates have been
    adjusted based on data submitted by the Regional Districts.
...
-   The Powell River Regional District is now called qathet Regional
    District.
...
-   [Solid Waste Management Planning in
...
-   [Solid Waste Generation in British Columbia: 2010-2025 Forecast
    ...
    Stats (2012)](http://www.bcstats.gov.bc.ca/StatisticsBySubject/EnvironmentalStatistics/Analysis.aspx)
-   ^1^[British Columbia Ministry of Environment and Climate Change
    Strategy 2019/20 -- 2021/22 Service Plan (February 2019)
    (PDF)](https://www.bcbudget.gov.bc.ca/2019/sp/pdf/ministry/env.pdf)
